################################################################################
# Per-student randomized credentials.
#
# Each student gets:
#   - a unique Domain Admin password
#   - a unique teamserver password per C2 framework (Adaptix, Mythic, BRC4)
#   - a per-CDN (X-Api-<RandomName>, UUID) pair per C2 framework. Five
#     CDNs per stack: azure, cloudfront, workers, fastly, other. The
#     redirector's nginx maps each header → upstream port
#     (8443/8444/8445/8446/8447).
#
# All three C2 frameworks are per-student. BRC4 license caps the range
# at one teamserver activation, so c2-brc4 scenarios always run with
...
resource "random_password" "domain_admin" {
  ...
  # or Add-Computer credential parsing.
  override_special = "!#$%&*()-_=+"
...
# Sliver operator password — drives the multiplayer config-file
# generation seed. Alphanum-only; sliver-server's `new-operator`
# embeds it into the .cfg JSON.
...
# "Lab!Guac1" default which is trivially brute-forceable. We now
# generate a strong per-deploy random by default; the scenario can
# still pin a specific value via services.guacamole.admin_password
# (anything other than empty / the legacy "Lab!Guac1" default wins).
...
resource "random_password" "operator_ian_mythic" {
  ...
  numeric     = true
  special     = true
  # JSON-safe: no backslash, no quotes; Mythic's POST /operators body
  # is JSON. Defense against shell-escaping in the role's URI module.
  override_special = "!@#%^&*-_+="
}
