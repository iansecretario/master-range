...
locals {
  ...
  # AFD FQDN per (student, fronted-role). When AFD is enabled and the
  ...
  azure_callback_for = {
    ...
    stack_role => {
      ...
      sid => (
        ...
        # AFD path (operator-side / advanced_c2 enabled): use the
        # per-redirector AFD endpoint's *.azurefd.net hostname so beacon
        # configs don't leak the operator's custom-domain DNS zone.
        ? azurerm_cdn_frontdoor_endpoint.redirector[local.redirector_for["${sid}-${stack_role}"]].host_name
        # Local-IP fallback (student-redteam-lab and any deploy with
        # advanced_c2 disabled): resolve to the per-student teamserver's
        # private IP directly. Beacons stay inside the VNet, no AFD edge
    ...
    sid => [
      for cdn in local.cdn_names : {
        ...
        config = {
          ...
          callback_addresses = [
            cdn == "azure"
            ? "${local.azure_callback_for["c2-server"][sid]}:443"
          ...
          # detection exercise. AFD routes /* and the c2-redirector is a
    ...
    sid => [
      ...
      },
      {
        ...
        config = {
          host_bind = "0.0.0.0"
          port_bind = 8448
          # Same AFD callback as the BeaconHTTP `azure` listener but
          ...
          callback_addresses = "${local.azure_callback_for["c2-server"][sid]}:8448"
          encrypt_key        = local.adaptix_listener_enc_keys[sid]["azure"]
      ...
      # without needing to bring mTLS cert plumbing online.
      {
        ...
        config = {
          host_bind          = "0.0.0.0"
          port_bind          = 8449
          callback_addresses = "${local.azure_callback_for["c2-server"][sid]}:8449"
          encrypt_key        = local.adaptix_listener_enc_keys[sid]["azure"]
          ssl                = false
          tcp_banner         = ""
          error_answer       = "<!doctype html><title>404</title><h1>Not Found</h1>"
...
locals {
  ...
  brc4_callback_host = {
    for sid in local.students :
    sid => {
      azure      = local.azure_callback_for["c2-brc4"][sid]
