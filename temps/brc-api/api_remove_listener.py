...
async def main():
    parser = argparse.ArgumentParser(
        description="Remove a listener from the ratel server",
        epilog="Example:\n python3 api_remove_listener.py -user ninja -password pass@123 -handler 172.16.219.1:8443 -listener api-created-dns",
    ...
    parser.add_argument('-listener', type=str, required=True, help="listener name to remove", metavar='')
    args = parser.parse_args()

    wsClient = await bruteratel.br_connect_handler(args.user, args.password, args.handler)
    print("[+] Authentication success")

    if (await bruteratel.br_stop_listener(wsClient, args.listener)):
        print("[+] Listener stopped:", args.listener)
    else:
