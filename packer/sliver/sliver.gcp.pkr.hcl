################################################################################
# Packer template: Sliver C2 teamserver pre-baked on Debian 12 for the
# terra-range "sliver" role. GCP equivalent of sliver.pkr.hcl — same
# scripts/sliver-baseline.sh + shared linux-deprovision.sh; `googlecompute`
# source replaces `azure-arm`.
#
# Usage:
#   packer init  packer/sliver/sliver.gcp.pkr.hcl
#   packer build -var gcp_project_id=$TERRARANGE_GCP_HOST_PROJECT_ID \
#                packer/sliver/sliver.gcp.pkr.hcl
################################################################################
...
build {
  sources = ["source.googlecompute.sliver"]

  # Step 1: install Sliver server + dependencies + mingw cross-build deps.
  provisioner "shell" {
